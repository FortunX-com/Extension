...
function onExtensionIconClick(callback) {
    document.addEventListener('_listener_on_extension_icon_clicked', function () {
        callback();
    });

    // document.dispatchEvent(new CustomEvent('_dispatch_on_extension_icon_clicked'));
}
...
/*function openNewTab(url) {
	chrome.tabs.create({selected: false},function(tab){
		isTabOpen = true;
		openedTab = tab.id;
		chrome.tabs.update(openedTab, { url: url.toString() });
...
function getCountOccurrences(str, match) {
    ...
    let count;
    for (let i = count = 0; i < str.length; count += +(match === str[i++]));
    return count;
}
