...
document.addEventListener('getDataFromStorage', function (e) {
    ...
    chrome.storage.local.get([key], function (items) {
        ...
        document.dispatchEvent(new CustomEvent(storage_counter + '_responseDataFromStorage', {detail: {'response': value}}));
    });
});
...
document.addEventListener('requestHttp', function (e) {
    ...
    fetchResource(url, init).then(r => r.text()).then(result => {
        document.dispatchEvent(new CustomEvent(http_counter + '_responseHttp', {detail: {'response': result}}));
    }).catch(error => {
        document.dispatchEvent(new CustomEvent(http_counter + '_errorHttp', {detail: {'response': error}}));
    });
});
...
function detectMessageType(request, sender, sendResponse) {
    ...
    if (request && request.type) {
        ...
        switch (request.type) {
            ...
            case 'on_extension_icon_clicked': {
                document.dispatchEvent(new CustomEvent('_listener_on_extension_icon_clicked'));
                break;
            }
        }
    }
